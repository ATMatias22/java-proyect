...
import errores.LineaInvalidaException;
...
public class Controlador {
    ...
    public void iniciar() {
        try {
            ...
            String opcLinea = null;
            boolean error = true;
            do {
                try {
                    opcLinea = v.mostrarMenuSeleccionLinea(CORTE_PROGRAMA);
                    if (!opcLinea.equals(CORTE_PROGRAMA)) {
                        LineaSubte lineaSeleccionada = obtenerLineaSegunLetra(opcLinea);
                        subMenuSeleccionEstacion(lineaSeleccionada);
                        error = false;
                    }

                } catch (LineaInvalidaException lie) {
                    v.mostrarMensajeError(lie.getMessage());
                } catch (EstacionInvalidaException eie) {
                    v.mostrarMensajeError(eie.getMessage());
                }

            } while (!opcLinea.equalsIgnoreCase(CORTE_PROGRAMA) || error == true);
    ...
    private LineaSubte obtenerLineaSegunLetra(String letra) {
        ...
        if (lineaADevolver == null) {
            throw new LineaInvalidaException("LINEA " + letra + " NO EXISTE");
        }
